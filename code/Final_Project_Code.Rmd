...
During week 9 I received 2 main pieces of feedback from my group: consider changing the represenation of sleep efficiency through opacity so that differences in efficiency are easier to see visaully & add something to it that relates to your theme of tracking sleep data. As a result, I scaled my efficiency scores using the equation $opacity=0.2+\frac{x-0.51}{0.95-0.51}*0.8$ (maximum efficiency = 95%, minimum efficiency = 51%) to better emphasize differences in efficiency and added a night sky background behind my color strips to coincide with the topic of my data as well as increase its creativity.
...
sleep_lm <- lm(time_slept_minutes ~ sleep_efficiency, data = personal_data_clean)
...
ggplot(personal_data_clean, 
       aes(x = sleep_efficiency, y = time_slept_minutes)) +
  geom_point(color = "darkgreen", size = 2.5, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  labs(
    x = "Sleep efficiency (%)",
    y = "Time slept (minutes)",
    title = "Relationship between sleep efficiency and time slept"
  ) +
  theme_minimal()
```
there is one outlier I will remove it and recheck assumptions 
```{r}
personal_data_no_outlier <- personal_data_clean |>
  filter(time_slept_minutes != 230)  # remove the outlier

updated_sleep_lm <- lm(time_slept_minutes ~ sleep_efficiency, data = personal_data_no_outlier)
...
# scatterplot to assess linearity
ggplot(personal_data_no_outlier, 
       aes(x = sleep_efficiency, y = time_slept_minutes)) +
  geom_point(color = "darkgreen", size = 2.5, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  labs(
...
The linearity assumption appears reasonably met because the residuals vs fitted plot does not show a strong curved pattern, although there is some scatter and a few unusual points. The normality and homoscedasticity (consistentcy across values) assumptions are reasonably satisfied although the Q–Q plot shows some deviation in the tails, and the scale-location plot suggests some uneven spread in residuals.
...
# create plot
ggplot(personal_data_no_outlier,
       aes(x = sleep_efficiency,
           y = time_slept_minutes)) +
  geom_point(
    color = "magenta",
    size = 3,
    alpha = 0.6
  ...
  labs(
    x = "Sleep efficiency (%)",
    y = "Time slept (minutes)",
    title = "Time slept increases with sleep efficiency"
  ) +
...
**Figure 2. Time slept increases with sleep efficiency.** Pink circles/points represent individual nightly observations of time slept (minutes) plotted against sleep efficiency (a percentage out of 100 as calculated by an Oura ringe). The purple line represents the fitted linear regression model predicting time slept from sleep efficiency, while the light blue shaded ribbon shows the 95% confidence interval around the model predictions. Together, the points, line, and shaded confidence band illustrate a statistically significant positive relationship ($\alpha$=0.05) between sleep efficiency and total sleep duration.
...
The linear regression showed a significant positive relationship between sleep efficiency and time slept (Linear regression, F(1, 53) =  27.7, $R^2$ = 0.33 , p < 0.001, $\alpha$ = 0.05). Based on model predictions, for each 1 unit (1 %) increase in sleep efficiency, we expect an increase in time slept of 5.83 ± 1.107 (SE) minutes. The model had an adjusted $R^2$ of 0.331, meaning sleep efficiency explained about 33.1% of the variation in time slept, which indicates a moderate effect size.
